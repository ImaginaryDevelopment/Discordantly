...
module FunctionalHelpersAuto =
    ...
    // unsafe null check
    let inline isNullU x =
        match box x with
        | null -> true
        | _ -> false
    let inline isValue x = isNullU x |> not
...
//        | i -> x.Substring(i + delimiter.Length)
    ...
    let toCamelCase s = // https://github.com/ayoung/Newtonsoft.Json/blob/master/Newtonsoft.Json/Utilities/StringUtils.cs
        if String.IsNullOrEmpty s then
            ...
            let camelCase = Char.ToLower(s.[0], System.Globalization.CultureInfo.InvariantCulture) |> fun x -> x.ToString(System.Globalization.CultureInfo.InvariantCulture)
            if (s.Length > 1) then
...
module StringPatterns =
    ...
    //    | _ -> ValueString s
    let (|StartsWith|_|) d = Option.ofValueString >> Option.filter (String.startsWith d) >> Option.map ignore
    let (|StartsWithI|_|) d = Option.ofValueString >> Option.filter (String.startsWithI d) >> Option.map ignore
    let (|After|_|) d =
    ...
    let inline (|TryParser|_|) parser (s:string) : 't option =
        match parser s with
        | true, x -> Some x
        | _ -> None

    let inline (|IsTOrTryParse|_|) (parser) (x:obj): 't option =
        match x with
        | null -> None
        // Nullables will downcast to their value type if they have a value inside, so no need to match on Nullable
        | :? 't as x -> Some x
        | :? option<'t> as x when Option.isSome x -> x
        | :? string as p ->
            match parser p with
            | true, v -> Some v
            | _, _ -> None
    ...
    let (|ParseFloat|_|) x = x |> (|TryParser|_|) Double.TryParse |> Option.map LanguagePrimitives.FloatWithMeasure
    let (|ParseInt|_|) x = x |> (|TryParser|_|) Int32.TryParse |> Option.map LanguagePrimitives.Int32WithMeasure
    let (|ParseDateTime|_|) = (|TryParser|_|) DateTime.TryParse
    let (|ParseDecimal|_|) x = x |> (|TryParser|_|) Decimal.TryParse |> Option.map LanguagePrimitives.DecimalWithMeasure
    let (|ParseBoolean|_|) = (|TryParser|_|) Boolean.TryParse
    let (|ParseInt64|_|) x = x |> (|TryParser|_|) System.Int64.TryParse |> Option.map LanguagePrimitives.Int64WithMeasure
    let (|ParseGuid|_|) = (|TryParser|_|) System.Guid.TryParse

    let (|Float|_|) (x:obj) = (|IsTOrTryParse|_|) Double.TryParse x |> Option.map LanguagePrimitives.FloatWithMeasure
    let (|Decimal|_|) (x:obj) = (|IsTOrTryParse|_|) Decimal.TryParse x |> Option.map LanguagePrimitives.DecimalWithMeasure
    let (|Int|_|) (x:obj) = (|IsTOrTryParse|_|) Int32.TryParse x |> Option.map LanguagePrimitives.Int32WithMeasure
    let (|Boolean|_|) (x :obj) = (|IsTOrTryParse|_|) Boolean.TryParse x
    let (|DateTime|_|) (x :obj) = (|IsTOrTryParse|_|) DateTime.TryParse x
...
module ReadOnlyCollection =
    let empty<'t> : ReadOnlyCollection<'t> = System.Collections.ObjectModel.ReadOnlyCollection<'t>(ResizeArray())
    let ofSeq (x:_ seq) = x |> ResizeArray |> ReadOnlyCollection<_>

module IReadOnlyCollection =
    let empty<'t> : IReadOnlyCollection<'t> = upcast list.Empty
module IReadOnlyList =
    let empty<'t> : IReadOnlyList<'t> = upcast list.Empty
    let ofSeq (x:'t seq):IReadOnlyList<'t> = x |> List.ofSeq :> _
...
module Railway =
    ...
    //let map' f = bind (f >> Happy)

    let mapUnhappy f =
        either Happy (f >> Unhappy)
...
module Diagnostics =
    ...
    let logS topic (attrs:AttrDict) s =
        ...
        try
            fileLog s
        with ex ->
            printfn "Exception attempting to log:%A" ex
            try
    ...
    let makeDummyDisposable = lazy(disposable ignore)
    ...
    let BeginLogScope scopeName=
        ...
        |> function
            | Some x -> x
            | None -> makeDummyDisposable.Value
    type ITimedScope =
        inherit IDisposable
    ...
    module TimerInternals =
        ...
        type TimedScopeHandler (scopeName) =
            ...
            member __.MarkElapsed(name) =
                if not isDisposed then
                    logElapsed (sprintf "%s_%s" scopeName name) sw
            member __.Stop() = if not isDisposed then sw.Stop()
            member __.Resume() = if not isDisposed then sw.Start()
    ...
    module Unboxing =
        ...
        let getBoolMaybe (o:obj) =
            match o with
                ...
                | _ -> None
    let rec compareProps goDeep asTypeOpt nolist nameOpt expected actual =
        ...
        let t = asTypeOpt |> Option.getOrDefault' (lazy(expected.GetType()))
        let props = t.GetProperties()
        let toWalk = props |> Seq.filter(fun p -> nolist |> Seq.contains p.Name |> not && p.GetMethod.GetParameters().Length = 0)
        let nolist = nolist |> Seq.except (props |> Seq.map (fun p -> p.Name)) |> List.ofSeq
        seq{
            for p in toWalk do
                ...
                | Some (expected,actual) ->
                    if goDeep
                        ...
                        yield! compareProps true (Some p.PropertyType) nolist (Some bindingName) expected actual
                    let bindingName = match nameOpt with | Some n -> sprintf "%s.%s" n p.Name | None -> p.Name
...
//            let gtd = if typ.IsGenericType then typ.GetGenericTypeDefinition() |> Some else None
    ...
    // instead of null in TypeOf or IsTypeOf matches for types that don't allow null
    ...
    [<NoComparison>]
    type GeneratedAlternate = {Value:obj;NonEqualValue:obj}

    [<NoEquality;NoComparison>]
    type PropSafer<'T> = {TargetObj:'T;PropertyInfo:PropertyInfo;Getter:unit -> obj; Setter:obj -> unit }
    let generatePropSafer x (prop:PropertyInfo) : PropSafer<_> option =
        if prop.CanRead && prop.CanWrite then
            {TargetObj=x;PropertyInfo = prop; Getter=(fun () -> prop.GetValue x);Setter=fun value -> prop.SetValue(x,value)}
            |> Some
        else None
    //[<NoEquality;NoComparison>]
    //type PropSafer<'T,'TProp> = {Target:'T;PropertyInfo:PropertyInfo;Getter:(unit -> 'TProp) option; Setter:('TProp -> unit) option}

    // we can't use DateTime.Min or Max since they may magically turn into nulls when passed
    let defDt,altDt = DateTime(1999,1,1,0,0,0), DateTime(2000,1,1,0,0,0)
    let generateNullableT (existingValue:obj,targetType) : GeneratedAlternate option =
        ...
        |> Option.bind(fun t ->
            ...
            |IsNull ->
                ...
                | TypeOf(isType:DateTime) -> defDt |> box |> Some
                | TypeOf(isType:bool) -> true |> box |> Some
                | _ -> None
            | :? int as i -> (if i = 2 then 3 else 2) |> box |> Some
            | :? DateTime as dt ->
                if dt = defDt then altDt else defDt
                |> box
                |> Some
            | :? bool as b -> not b |> box |> Some
            | _ -> None
        )
        |> Option.map(fun x -> {Value=existingValue;NonEqualValue=x})


    let (|NullableValueT|_|) : obj*Type -> GeneratedAlternate option = generateNullableT

    //// goal take Nullable type value (possibly null) with the target type and generate something that is not equal to the existing value
    let generateValue debug : obj * Type -> GeneratedAlternate option =
        function
        |NullableValueT x -> Some x
        | null, TypeOf(isType:string) ->
            Some {Value=null;NonEqualValue="hello"}
        | :? string as x, TypeOf(isType:string) ->
            Some {Value = x; NonEqualValue= if x = "hello" then "world" else "hello"}
        | Int i, TypeOf (isType:int) ->
            let x = [2..4] |> Seq.find((<>) i)
            Some {Value=i;NonEqualValue=x}
        | Boolean x, TypeOf(isType:Nullable<bool>)
        | Boolean x, TypeOf(isType:bool) ->
            Some {Value=x;NonEqualValue=not x}
        | :? DateTime as x, TypeOf(isType:DateTime) ->
            Some {Value=x;NonEqualValue= if x = defDt then altDt else defDt}
        | :? Guid as x, TypeOf(isType:Guid) ->
            let y = Guid.NewGuid()
            if x = y then
                Guid.NewGuid()
            else y
            |> fun v -> Some {Value=x;NonEqualValue=v}

        | null, TypeOf(isType:byte[]) ->
            Array.empty<byte>
            |> fun v -> Some {Value=null;NonEqualValue=v}
        | null, pt ->
            if debug then
                invalidOp <| sprintf "No match for null, %s" (getTypeString pt)
            None
        | _,pt ->
            if debug then
                invalidOp <| sprintf "No match for null, %s" (getTypeString pt)
    ...
    let getTypes t =
        let rec getTypes (t:Type) =
            seq{
                yield t
                yield! t.GetInterfaces() |> Seq.collect getTypes
                if not <| isNull t.BaseType then
                    yield! getTypes t.BaseType
            }
        getTypes t

    let getTypesT<'t> : Type seq =
        let t = typeof<'t>
        getTypes t
...
module Inpc =
    ...
    let listenHere f (x:INotifyPropertyChanged) =
        let disp = x.PropertyChanged.Subscribe(fun p ->
            f p.PropertyName
        )
        disp
...
module Dynamics =
    ...
    let inline makePropAccess (flags:BindingFlags) =
    ...
    module PrivateProperty =
        let inline (?) this key =
            let (?) = makePropAccess (BindingFlags.NonPublic ||| BindingFlags.Instance)
            // parens ARE required
            this?(key)
